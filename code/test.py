...
def single_layer_similar_heatmap_visual(output_t0, output_t1, save_change_map_dir, epoch, filename, layer_flag, dist_flag):
    ...
    interp = nn.Upsample(
        size=[cfg.TRANSFROM_SCALES[1], cfg.TRANSFROM_SCALES[0]], mode='bilinear')
    ...
    similar_distance_map_rz = interp(Variable(torch.from_numpy(
        similar_distance_map[np.newaxis, np.newaxis, :])))
    ...
    def internal():
        ...
        if base_seg_model == 'deeplab':
            ...
            pretrain_deeplab_path = os.path.join(
                cfg.PRETRAIN_MODEL_PATH, 'deeplab_v2_voc12.pth')
            ...
            pretrain_vgg_path = os.path.join(
                cfg.PRETRAIN_MODEL_PATH, 'vgg16_from_caffe.pth')
        ...
        optimizer = torch.optim.SGD(
            [
                {'params': set_base_learning_rate_for_multi_layer(
                    model), 'lr': cfg.INIT_LEARNING_RATE},
                {'params': set_2x_learning_rate_for_multi_layer(
                    model), 'lr': 2 * cfg.INIT_LEARNING_RATE, 'weight_decay': 0},
                {'params': set_10x_learning_rate_for_multi_layer(
                    model), 'lr': 10 * cfg.INIT_LEARNING_RATE},
                {'params': set_20x_learning_rate_for_multi_layer(
                    model), 'lr': 20 * cfg.INIT_LEARNING_RATE, 'weight_decay': 0}
            ], lr=cfg.INIT_LEARNING_RATE, momentum=cfg.MOMENTUM, weight_decay=cfg.DECAY)
        ######## iter img_label pairs ###########
        loss_total = 0

    r1 = random.randint(1, 16)
    file = "/home/siddhant/test/" + str(r1) + ".png"
    shutil.copy(file, 'results')
    img = Image.open("results/" + str(r1) + ".png")

    def another():
        for epoch in range(100):
            for batch_idx, batch in enumerate(train_loader):
                step = epoch * len(train_loader) + batch_idx
                util.adjust_learning_rate(
                    cfg.INIT_LEARNING_RATE, optimizer, step)
                model.train()
                ...
                out_fc_t0, out_fc_t1 = out_fc
                out_embedding_t0, out_embedding_t1 = out_embedding
                label_rz_conv5 = Variable(util.resize_label(label.data.cpu(
                ).numpy(), size=out_conv5_t0.data.cpu().numpy().shape[2:]).cuda())
                label_rz_fc = Variable(util.resize_label(
                    label.data.cpu().numpy(), size=out_fc_t0.data.cpu().numpy().shape[2:]).cuda())
                label_rz_embedding = Variable(util.resize_label(label.data.cpu(
                ).numpy(), size=out_embedding_t0.data.cpu().numpy().shape[2:]).cuda())
                contractive_loss_conv5 = MaskLoss(
                    out_conv5_t0, out_conv5_t1, label_rz_conv5)
                contractive_loss_fc = MaskLoss(
                    out_fc_t0, out_fc_t1, label_rz_fc)
